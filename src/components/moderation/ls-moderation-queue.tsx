...
interface ResolvedSectionProps {
	resolvedReports: Array<{ reportId: number; reporterName: string | null; reportedUserName: string | null; type: string | null }> | null;
	isLoading: boolean;
	onLoad: () => void;
	searchName: string;
	...
	allTypes: string[];
	renderCard: (report: any) => React.ReactNode;
}
	...
	return (
		<Stack gap="md" mt="xl">
			...
			<TextInput
				...
				rightSection={
					<Button size="compact-sm" variant="subtle" color="gray" onClick={onCommitSearch}>
						Search
					</Button>
				}
				...
				<Group gap="xs" wrap="wrap">
					{allTypes.map((type) => (
						<Badge
							...
							style={{ cursor: "pointer", maxWidth: "100%", whiteSpace: "normal", wordBreak: "break-word" }}
			...
			{resolvedReports === null ? (
				<Button variant="subtle" color="gray" size="sm" onClick={onLoad} loading={isLoading}>
		...
		<Tabs value={activeTab} onChange={setActiveTab}>
			...
			<Tabs.Panel value="feed" pt="md">
				{feedReports.length === 0 ? (
					<Text c="gray.5">No open feed reports.</Text>
				) : (
					<Stack>
			...
			<Tabs.Panel value="users" pt="md">
				{userReports.length === 0 ? (
					<Text c="gray.5">No open user reports.</Text>
				) : (
					<Stack>
